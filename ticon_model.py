...
def scaled_dot_product_attention_custom(query, key, value, attn_bias=None, attn_mask=None, dropout_p=0.0,
...
class Attention(nn.Module):
	def __init__(
		...
		self.slopes =  slopes[None, :, None, None] #einops.rearrange(slopes, 'b -> 1 b 1 1')
	...
	def forward(
		...
		corrds_expanded = coords.unsqueeze(2).expand(-1, -1, n_k, -1)  # (b, m, d) -> (b, m, 1, d) -> (b, m, n, d)
		context_coords_expanded = context_coords.unsqueeze(1).expand(-1, n_q, -1, -1)
		euclid_dist = torch.sqrt(torch.sum((corrds_expanded - context_coords_expanded) ** 2, dim=-1))
...
class NaiveResidual(nn.Module):
	def __init__(self, drop_prob: float | int, norm: nn.Module, fn: nn.Module, gamma: nn.Parameter):
		super().__init__()
		self.norm = norm
		self.fn = fn
...
class EncoderDecoder(nn.Module):
	def __init__(
		...
		for i in range(len(in_dims)):
			self.input_proj_dict[f"input_proj_{self.tile_encoder_keys[i]}"] = ProjectionMlp(in_features=in_dims[i], hidden_features=self.encoder.embed_dim, out_features=self.encoder.embed_dim)
			
		for i in range(self.num_decoders):
			self.decoder_dict[f"decoder_{i}"] = nn.ModuleDict({})
			self.decoder_dict[f"decoder_{i}"]["transformer"] = Transformer(
									**transformers_kwargs,
									**decoder_kwargs,
			...
			self.decoder_dict[f"decoder_{i}"]["norm"] = norm_layer(self.decoder_dict[f"decoder_{i}"]["transformer"].embed_dim, 
														**final_norm_kwargs)
			self.mask_dict[f"mask_token_{i}"] = nn.Parameter(torch.empty(1, self.decoder_dict[f"decoder_{i}"]["transformer"].embed_dim))
			
		for i in range(len(self.decoder_out_dims)):
			self.output_proj_dict[f"output_proj_{self.tile_encoder_keys[i]}"] = ProjectionMlp(in_features=self.encoder.embed_dim, hidden_features=self.encoder.embed_dim, out_features=self.decoder_out_dims[i])
	...
	def forward_features(
		...
		if dec_layer is not None:
			...
			for dec_index in range(self.num_decoders):
				decoder_outputs = self.decoder_dict[f"decoder_{dec_index}"]["transformer"](
					self.mask_dict[f"mask_token_{dec_index}"][None].expand(*coords_dec.shape[:2], -1),
					contexts=[encoder_outputs[len(self.encoder.blocks)]] * self.decoder_dict[f"decoder_{dec_index}"]["transformer"].n_blocks,
					coords=coords_dec,
					context_coords=coords_enc,
					return_layers={dec_layer},
				)
				dec_output = self.decoder_dict[f"decoder_{dec_index}"]["norm"](decoder_outputs[dec_layer])
				
				for out_index in range(len(self.decoder_out_dims)):
					dec_final_output[self.tile_encoder_keys[out_index]] = self.output_proj_dict[f"output_proj_{self.tile_encoder_keys[out_index]}"](dec_output)
		else:
	...
	def forward(
		...
		enc_output, dec_output = self.forward_features(x, 
...
def __model_loader__(
	...
	config_override: Mapping = {"model":{"num_decoders": 0, "decoder_out_dims":[]}},
	device: str = "cuda",
) -> nn.Module:
	...
	print(f"Loading model with cfg {OmegaConf.to_yaml(cfg)}")
	...
	if isinstance(pretrained_weights, str) and pretrained_weights != "":
		...
		raise Exception(f"checkpoint not available")
	model.eval()
	print("return")
	return model
